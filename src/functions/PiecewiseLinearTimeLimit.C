/****************************************************************/
/*               DO NOT MODIFY THIS HEADER                      */
/* MOOSE - Multiphysics Object Oriented Simulation Environment  */
/*                                                              */
/*           (c) 2010 Battelle Energy Alliance, LLC             */
/*                   ALL RIGHTS RESERVED                        */
/*                                                              */
/*          Prepared by Battelle Energy Alliance, LLC           */
/*            Under Contract No. DE-AC07-05ID14517              */
/*            With the U. S. Department of Energy               */
/*                                                              */
/*            See COPYRIGHT for full restrictions               */
/****************************************************************/

#include "PiecewiseLinearTimeLimit.h"

template<>
InputParameters validParams<PiecewiseLinearTimeLimit>()
{
  InputParameters params = validParams<Piecewise>();
  params.addParam<Real>("tlimit",1.0,"set lifetime for a function [s]");
  return params;
}

PiecewiseLinearTimeLimit::PiecewiseLinearTimeLimit(const InputParameters & parameters) :
  Piecewise(parameters),
  _t_limit(getParam<Real>("tlimit"))
{
}

PiecewiseLinearTimeLimit::~PiecewiseLinearTimeLimit()
{
}

Real
PiecewiseLinearTimeLimit::value(Real t, const Point & p)
{
  Real func_value;
  if (_has_axis)
  {
    func_value = _linear_interp->sample( p(_axis) );
  }
  else
  {
    func_value = _linear_interp->sample( t );
  }
  if(t >= _t_limit) return 0.0;//add a time limit to this function
  return _scale_factor * func_value;
}

Real
PiecewiseLinearTimeLimit::timeDerivative(Real t, const Point & p)
{
  Real func_value;
  if (_has_axis)
  {
    func_value = _linear_interp->sampleDerivative( p(_axis) );
  }
  else
  {
    func_value = _linear_interp->sampleDerivative( t );
  }
  return _scale_factor * func_value;
}

Real
PiecewiseLinearTimeLimit::integral()
{
  return _scale_factor * _linear_interp->integrate();
}

Real
PiecewiseLinearTimeLimit::average()
{
  return integral()/(_linear_interp->domain(_linear_interp->getSampleSize()-1)-_linear_interp->domain(0));
}

